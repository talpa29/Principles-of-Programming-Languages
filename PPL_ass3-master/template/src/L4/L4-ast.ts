...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => 
        params.length !== 1 ? makeFailure ("cant be big then 1") :
        bind(mapResult(parseL4CExp, params), (x : CExp[]) =>  
        isVarRef(x[0]) ? makeOk(makeTraceExp(x[0])) : 
        makeFailure ("cant be big then 1"));

...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, getFBindingVal, isFBinding, makeFBinding, theClock } from "./L4-env-box";
...
import { Result, bind, mapv, mapResult, makeFailure, makeOk, safe2, isOk } from "../shared/result";
...
const evalTraceExp = (exp: TraceExp): Result<void> => 
    bind(applyEnvBdg(theGlobalEnv,exp.var.var),(x: FBinding) => isClosure(getFBindingVal(x)) ? 
    makeOk(setFBinding(x,makeTracedClosure(getFBindingVal(x) as Closure ,exp.var.var))) : 
    makeFailure<void>("failure"));
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    const vars = map((v: VarDecl) => v.var, proc.params);
    printPreTrace(proc.name,args,age);
    age++;
    const ret = evalSequence(proc.body, makeExtEnv(vars, args, proc.env));
    const ret_val = isOk(ret) ? ret.value : "none";
    age--;
    printPostTrace(ret_val, age);
    return ret;
}

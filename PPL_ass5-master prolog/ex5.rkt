...
(define pipe$  
  (lambda (fs cont)
    (if (empty? (cdr fs))
      (cont (car fs))
    (pipe$ (cdr fs) 
      (lambda (res) (compose$ (car fs) res cont))))))
...
(define reduce-prim$ 
  (lambda (re init lst cont)
    (if (empty? (cdr lst))
        (cont (re init (car lst)))
        (reduce-prim$ re init (cdr lst) 
          (lambda (res) (cont (re res (car lst))))))))

; Signature: reduce-user$(reducer, init, lst, cont)
; Type: [[T1*T1*[T1->T1]->T1]*T1*[T1,T1,T1,....,T1]*[T1->T1]->T1]
; Purpose: Returns the reduced value of the given list, from left 
;          to right, with cont post-processing
...
(define reduce-user$ 
  (lambda (re$ init lst cont)
    (if (empty? (cdr lst))
        (re$ init (car lst) cont)
        (reduce-user$ re$ init (cdr lst) 
        (lambda (res) (re$ res (car lst) cont))))))
...
(define take-while-lzl 
        (lambda (lzl pred)
                (if (or (not (pred (head lzl))) (empty-lzl? lzl))
                    empty-lzl
                    (cons (head lzl) (lambda ()
                                              (take-while-lzl (tail lzl) pred))))))
